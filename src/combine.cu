...
//            }
        ...
        for (int k = 0; k < TILE; k++) {
            printf("tile %d: adding values a_shared[%d][%d] and b_shared[%d][%d] to out[%d][%d]; res before: %f; aval: %f, bval: %f\n", tile_idx, thread_x, k, k, thread_y, row, col, res, a_shared[thread_x][k], b_shared[k][thread_y]);
